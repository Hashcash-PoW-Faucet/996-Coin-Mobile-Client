...
// NOTE: This file intentionally implements generation/import/export first.
// TX building/signing should be added later.
// =========================
import 'dart:math';
...
class WalletService {
  static const int pubkeyPrefix = 0x35; // 53
  static const int wifPrefix = 0x80; // 128
  ...
  Uint8List privateKeyFromCompressedWif(String wif) {
    ...
    if (payload.length != 34) {
      throw Exception('Invalid compressed WIF length.');
    }
    if (payload[0] != wifPrefix) {
      throw Exception('Invalid WIF network prefix.');
    }
    if (payload[33] != compressedFlag) {
      throw Exception('Only compressed WIF is supported.');
    }
  ...
  Uint8List addressToPubKeyHash(String address) {
    ...
    if (payload.length != 21) {
      throw Exception('Invalid address payload length.');
    }
    if (payload[0] != pubkeyPrefix) {
      throw Exception('Invalid 996 address prefix.');
    }
